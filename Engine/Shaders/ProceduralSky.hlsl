...
ConstantBuffer<ProceduralSky> SkyCB;
...
{
    float4 clip = float4(2 * Input.UV.x - 1, 1 - 2 * Input.UV.y, 1, 1);
    float3 vWorldPosition = (mul(ViewCB.ViewProjectInverse, clip)).xyz;

    //this can be done in a VS

    float vSunE = sunIntensity(dot(SkyCB.SunDirection, up));
    float vSunfade = 1.0 - clamp(1.0 - exp((SkyCB.SunDirection.y)), 0.0, 1.0);
    float rayleighCoefficient = SkyCB.Rayleigh - (1.0 * (1.0 - vSunfade));
    ...
    // mie coefficients
    float3 vBetaM = totalMie(SkyCB.Turbidity) * SkyCB.MieCoefficient;
    ...
    // in scattering
    float cosTheta = dot(normalize(vWorldPosition - cameraPos), SkyCB.SunDirection);

    float rPhase = rayleighPhase(cosTheta * 0.5 + 0.5);
    float3 betaRTheta = vBetaR * rPhase;

    float mPhase = hgPhase(cosTheta, SkyCB.MieDirectionalG);
    float3 betaMTheta = vBetaM * mPhase;

    float3 Lin = pow(abs(vSunE * ((betaRTheta + betaMTheta) / (vBetaR + vBetaM)) * (1.0 - Fex)), float3(1.5, 1.5, 1.5));
    Lin *= lerp(float3(1.0, 1.0, 1.0), pow(vSunE * ((betaRTheta + betaMTheta) / (vBetaR + vBetaM)) * Fex, float3(1.0 / 2.0, 1.0 / 2.0, 1.0 / 2.0)), clamp(pow(1.0 - dot(up, SkyCB.SunDirection), 5.0), 0.0, 1.0));
	...
	//no tonemapped
    float3 color = (log2(2.0 / pow(SkyCB.Luminance, 4.0))) * texColor;
